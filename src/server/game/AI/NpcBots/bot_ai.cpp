...
void bot_ai::SetStats(bool force)
{
    ...
    if (me->GetLevel() != mylevel)
    {
        ...
        if (_killsCount > 0)
        {
            LOG_INFO("server.loading", "Bot leveled up! Bot: {}, prevlevel: {}, mylevel: {}, _killsCount: {}", me->GetEntry(), me->GetLevel(), mylevel, _killsCount);
            //LOG_INFO("server.loading", "mylevel >= 60, reverting to prevlevel");
            mylevel = me->GetLevel();
            _killsCount = 0; // Reset kills counter
        }
        ...
        case BATTLEGROUND_AV:
        {
            ...
            for (auto const& p : { std::pair{TEAM_ALLIANCE, std::array{AV_CPLACE_MINE_N_3, AV_CPLACE_MINE_S_3}}, std::pair{TEAM_HORDE, std::array{AV_CPLACE_MINE_S_3, AV_CPLACE_MINE_N_3}} })
            {
                ...
                for (BG_AV_CreaturePlace sptype : p.second)
                {
                    ...
                    {
                        WanderNode const* mineWP = ASSERT_NOTNULL(WanderNode::FindInZoneWPs(mboss->GetAreaId(), mine_pred));
